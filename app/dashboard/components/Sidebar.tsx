      ...
      <div className="h-24 flex items-center px-8 shrink-0 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-zinc-600 mt-1.5 leading-none uppercase tracking-[0.3em]">
              Cinemas
            </p>
          </div>
        </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                <item.icon
                  className={`w-4.5 h-4.5 shrink-0 transition-all duration-300 ${isActive ? "text-primary scale-110 drop-shadow-[0_0_8px_rgba(209,31,38,0.5)]" : "text-zinc-700 group-hover:text-zinc-400"}`}
                />
                <span
                  className={`text-[13px] font-bold tracking-tight transition-colors ${isActive ? "text-white" : "group-hover:text-zinc-200"}`}
                >
                  {item.label}
                </span>
              </div>
